...
The matrix covers the full index rather than a curated class list. Pagination,
filtering, and output budgets are part of each capability's contract.

## REST Catalog 1.11.0 operations

The OAuth token operation is intentionally implemented inside the auth provider,
not registered as a model-callable tool. All other released operations map to one
discoverable tool. Mutation tools are registered only after explicit operator opt-in.
...
`signRequest` remains internal because its response can contain signed authorization
material intended for a storage request, not for model consumption. The capability
is still covered and contract-tested in the REST client.

## Current-main extensions

These operations are not in the 1.11.0 release baseline and execute only when the
catalog advertises their exact endpoints.

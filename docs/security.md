...
| Unattributable catalog change               | One redacted `catalog.mutation` stderr line per attempted mutation with operation, identifier, idempotency key, status, outcome, duration, and correlation id | Audit-emission, read-suppression, and redaction tests             |
